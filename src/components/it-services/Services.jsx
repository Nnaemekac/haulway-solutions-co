...
const Services = () => {
  ...
  const [isContactVisible, setIsContactVisible] = useState(false); // State to track visibility
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ...
      { threshold: 0.3 } // Adjust threshold based on when you want it to trigger
    );
  ...
  useEffect(() => {
    if (isContactVisible) return; // Stop auto-scrolling when contact is visible
  ...
  useEffect(() => {
    if (isContactVisible) return; // Prevent scrolling when contact is visible
  ...
  return (
    <>
      <section className="xui-pt-3 xui-pb-6 xui-pos-relative xui-lg-max-w-800 xui-w-fluid-100 xui-mx-auto xui-d-flex">
        {/* Left Services */}
